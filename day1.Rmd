...
- Other repositories:
    ...
    + github can host R can also host R packages
...
- `plot` is the most basic plotting function in R
    + it can try and guess the best plot based on the data
...
- It is more usual to display count data in a barplot
    + e.g. the counts of the a particular categorical variable
```{r}
...
- A histogram is a popular way of visualising a distribution
...
- Sometimes we want to compare distributions between different groupings in our data
- For this we need to use the '*formula*' syntax
    + For now, `y ~ x` means put variable `y` on the *y* axis and `x` on the x axis
```{r}
...
- In the Day_1_scripts you will find the file `ozone.csv`
...
- What data types are present? Try to think of ways to create the following plots from the data
    ...
    + a histogram
    + boxplot of a continuous variable against a categorical variable
...
```{r}
plot(weather$Ozone,weather$Solar.R)
```
...
- `plot` comes with a whole array of arguments that can be set
    + see `?plot`
...
- Take one of the plots you produced in the previous section and experiment with adding axes and colours

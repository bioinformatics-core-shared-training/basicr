...
author: Mark Dunning, Suraj Menon, and Aiora Zabala. \n Original material by Robert Stojnić, Laurent Gatto, Rob Foy
  John Davey, Dávid Molnár and Ian Roberts
date: '`r format(Sys.time(), "Last modified: %d %b %Y")`'
output: 
  slidy_presentation:
    footer: "This work is licensed under the Creative Commons Attribution-ShareAlike 2.0. COURSE HOME: cambiotraining.github.io/r-intro/"
css: mystyle.css
toc: yes
...
- Gene clustering example data
    + gene expression values in ***data/gene.expression.txt***
    + gene information in ***data/gene.description.txt***
    + patient information in ***data/cancer.patients.txt***
...
if(!file.exists("data/gene.expression.txt")){
...
```{r echo=FALSE}
genes <- read.delim("Day_2_scripts/gene.description.txt",stringsAsFactors = FALSE)
head(genes)
dim(genes)
```
...
- create a new data frame combining the patient information with ESR1 expression level
    + e.g. 
```{r echo=FALSE}
df <- cbind(subjects, ER = as.numeric(genevals))
head(df)
```

- sort the rows in this data frame according to ESR1 expression, and write the results to a file
    + HINT: look-up the help for the `order` function

## Solution

```{r}
erNegPatients <- subjects$er == 0
write.table(evals[,erNegPatients], file="erNegativeExpression.txt")
```

```{r}
df <- cbind(subjects, ER = as.numeric(genevals))
df <- df[order(df$ER),]
write.table(df, file="patientsOrderedByESR1.txt")
```

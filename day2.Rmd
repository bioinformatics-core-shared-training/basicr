...
- something doesn't look correct though....
...
- the p-value is not encouraging either
...
- recall that we can use a vector to subset, or order, the columns of a data frame

```{r}
match(subjects$samplename,colnames(evals))[1:10]
```

```{r echo=FALSE}
mm <- match(subjects$samplename,colnames(evals))[1:10]
```

- the output indicates that column `r mm[1]` of the expression matrix represents the same data as row 1 of the `subjects` matrix
- column `r mm[2]` of the expression matrix represents the same data as row 2 of the `subjects` matrix
- etc....
...
- match the `probe` index as before
    + we haven't changed the rows so this should be the same
...
- Imagine if we have a list of 100 genes to test
    + what a pain it would to change the script for each gene

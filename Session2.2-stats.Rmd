...
As with `points`, we can only use `lines` to add to an existing plot
...
We firstly need to test if the variances of the two groups are comparable

- it is usually safer to insepct the boxplots, but a function is provided to aid us
- `var.test`
- for completeness, we include the code to create the patients data frame
    + you probably won't need this if you're following the whole course
...
The `var.test` function was testing the hypothesis that the variances are equal, but a non-significant p-value means we cannot reject this hypothesis.

## Perform the t-test

As said previously, `t.test` can also be used for two-group comparisons
- the function requires minimal alterations to perform a two-sample test

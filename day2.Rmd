...
- Modify the selectioncriteria for amplification to be > 30%
...
- A breast cancer dataset with numerous patient characteristics 
    ...
    + what genes show a statistically-significant different between ER groups?
...
subset <- evals[match(probes,rownames(evals)),]
```

## Plotting genes

```{r}
pdf("gene-boxplots.pdf")
for(i in 1:length(genelist)){
  boxplot(as.numeric(subset[i,])~factor(subjects$er),main=genelist[i],xlab="ER Status")
}
dev.off()
```
...
```{r}
heatmap(as.matrix(evals[match(probes,rownames(evals)),]))
```

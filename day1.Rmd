...
- Note that names are usually carried across to the new vector. Sometimes this is what we want (as for `coding.pc`) but sometimes it is not (when we are comparing human to yeast). We can remove names by setting them to the special `NULL` value:
...
- or use the `View()` function to get a neat display of the data:
```{r eval=FALSE}
View(rawData)
```
...
- Patients are *near normal* if: `(NB_Amp / Nuclei < 0.33 & NB_Del == 0)` (where `NB_Del` is the number of deletions)
1. Modify the condition in our previous code to find these patients
2. Write out a results file of the samples that match these criteria, and open it in a spreadsheet program
...
##Making a Scatter Plot
- R tries to guess the most appropriate way to visualise the data, according to the class and dimensions of the object(s) provided
...
- A histogram is a popular way of visualising a distribution of ***continuous*** data:
    ...
    + The y-axis can be either frequency of density (`freq` argument)
...
- Can also use **R**ed **G**reen **Blue** and hexadecimal values:
       + `rgb(0.7, 0.7, 0.7)` → A light grey in RGB values
...
plot(1:10, type="n", xlim = c(1,6), ylim=c(-1,5),axes=F,xlab="",ylab="")
points(x,y,pch=i, cex=3)
text(x,y-0.4,i, cex=2)
```

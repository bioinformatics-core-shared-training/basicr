...
- The basic data structure in R is a **vector** -- an ordered collection of values. R treats even single values as 1-element vectors. The function `c` *combines* its arguments into a vector:
...
- Packages on CRAN:
    + Use CRAN search to find functionality you need: http://cran.r-project.org/search.html
    + Or look for packages by theme: http://cran.r-project.org/web/views/
- Packages in Bioconductor -- specialised in genomics: http://www.bioconductor.org/packages/release/bioc/
- Other repositories:
...
- Data frames are R's speciality, but R also handles matrices:
    ...
    + some calculations are more efficient to do on matrices
...
We first have to locate the data in our hard drive

- The default location for R to look for data is the ***working directory***
...
- If the file you want to read is listed in the output of `list.files()`, then you will be able to use the filename to read the data
- Otherwise you will need the path to the file
- The `file.choose` function will help you to store the location of the file as a variable
```{r eval=FALSE}
myfile <- file.choose()
myfile
```
...
Always check that the data have been imported correctly

- Check dimensions and first few rows
- R may not always give an error message!
- A common cause of problems is choosing an incorrect separator
...
- ***You need to think about how best to represent your data*** 
...
- The boxplot is commonly-used in statistics to visualise a distribution
    ...
    + The top and bottom of the box is the 75th and 25th percentiles
